...
const diffChildren=(vnodeChildren,domNodeChildren)=>{
  const patches=[];
...
export const renderComponent = (component, parent) => {
  ...
  console.log(rendered,"my rendered component");
...
export const diff = (dom, vnode, parent) => {
  if (dom) {
    ...
    console.log(vnode.nodeName,dom.tagName);

    var lowerTagName=dom.tagName.toLowerCase();
    if(vnode.nodeName!==lowerTagName){
      console.log("converted because there is different tagName");
    ...
    console.log(vnode.attributes,"this is attributes",dom.attributes);
     var patch=diffAttributes(vnode.attributes,dom.attributes);
     var xyz=patch(dom);
    var patchChild=diffChildren(vnode.children,dom.childNodes);
    patchChild(dom);
    console.log(patchChild,"this is patching returned by child");
     console.log(xyz);
